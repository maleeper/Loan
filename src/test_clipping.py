...
def feature_engineering(df):
    df = df.copy()
    # Apply clipping BEFORE feature engineering or AFTER? 
    # Usually better to clip raw values so derived features are also stable.
...
def main():
    ...
    # Feature Engineering with Clipping
    print("Applying feature engineering with CLIPPING (99th percentile)...")
    ...
    print("Evaluating Ensemble with Clipping on subset (20k rows)...")
    ...
    print(f"Ensemble AUC with Clipping: {scores.mean():.4f} (+/- {scores.std():.4f})")
