...
from sklearn.preprocessing import OneHotEncoder, StandardScaler, OrdinalEncoder, FunctionTransformer, TargetEncoder
...
def get_preprocessor(X):
    ...
    # Changed from ordinal_features to target_features
    target_features = ['grade_subgrade']
    ...
    # Target Encoder instead of Ordinal Encoder
    target_transformer = Pipeline(steps=[
        ('imputer', SimpleImputer(strategy='most_frequent')),
        ('target', TargetEncoder(target_type='continuous', smooth=0.0)) # smooth=0.0 to trust data more
    ])
...
def main():
    ...
    print("Building Stacking Classifier with Target Encoding...")
    ...
    print("Evaluating Stacking Classifier with Target Encoding on subset (20k)...")
    ...
    print(f"Stacking AUC (Target Encoded): {scores.mean():.4f} (+/- {scores.std():.4f})")
